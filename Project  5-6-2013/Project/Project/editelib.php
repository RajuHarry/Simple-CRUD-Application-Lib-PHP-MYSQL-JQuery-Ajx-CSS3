...
{
	$id = $_GET['id'];
	$sel = mysql_query("select * from php where id=$id");
...
$update = "update php set title='$title',detail='$detail' where id=$id ";

$res=mysql_query($update);

echo '<script> location.href="addelib.php" </script>'; 
...
<script type="text/javascript">
		function Prompt()
  			{
  				alert("E-Library Record Updated sccsessfully");
  			}
...
<form  method="post" action="">
	<div style="background: rgba(255, 255, 255,0.3); margin-bottom:-20px;">
		...
		<h2 style=" color:#FFF; padding:20px 10px 0px;text-shadow: 0px 0px 5px #000; ">Elibrary Add Panel</h2>
			...
			<input  type="text" class="input"  style="background: rgba(255, 255, 255,0.5);" name="title" value="<?php echo $row['title']; ?>"/><br /><br />
			<span style=" color:#903; padding:20px 10px 0px;text-shadow: 0px 0px 5px #FFF; ">Content</span>
			<br /><br />
			<textarea  style="background: rgba(255, 255, 255,0.5);" name="detail"><?php echo $row['detail']; ?></textarea>
			<br />
